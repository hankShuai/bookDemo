    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(".updateCount").change(function () {
                 var product = $(this).parent().parent().find("td:first").text();
                 var count = $(this).val();

                if(confirm("你确定要将"+product+"商品数量修改为"+count+"吗？")){
...
<div id="main">
    ...
    <table>
        ...
        <c:if test="${empty sessionScope.cart.items}">
            <tr>
                <td colspan="5"><a href="index.jsp">请去挑选几件喜欢的商品吧</a></td>
            </tr>
        </c:if>
        <c:if test="${not empty sessionScope.cart.items}">
            <c:forEach items="${sessionScope.cart.items}" var="entry">
                <tr>
                    ...
                    <td><a class="delItem" href="cartServlet?action=delItem&id=${entry.value.id}">删除</a></td>
                </tr>
            </c:forEach>
        </c:if>
